...
class operadores_model extends MY_model {
    ...
    function getAlloperadores() {
        ...
        $return = $this->db->get('operators')->result();
        if (!empty($return)) {
            return $return;
        }
    ...
    function getAllOperadoresByParceiroid($id_company) {
        ...
        return $this->db->get('operators');
    }

    function  getAllDepartamentosByParceiroid($id_company) {
        $this->db->where('id_company', $id_company);
        return $this->db->get('departments');
    }

    function getAllStringRelacionamentoDepartamentoAndOperadorByOperadorid($id_operator) {
    ...
    function getOperador($data) {
        ...
        if (isset($data['id_operator']) && isset($data['id_company'])) {
            ...
            $this->db->where('id_operator', $data['id_operador']);
            $return = $this->db->get('operators')->row();
        }
        if (!empty($return)) {
            $return->password = $this->encrypt->decode($return->password);
    ...
    function hasoperatorExistsByLoginAndParceiroid($operator_login, $id_company) {
        ...
        $this->db->where('id_company', $id_company);
        $query = $this->db->get('operators')->num_rows;

        return $query > 0 ? true : false;
    }

    function hasoperatorExistsByIdoperatorAndParceiroid($id_operator, $id_company) {
        $this->db->where('id_operator', $id_operator);
        $this->db->where('id_company', $id_company);
        $query = $this->db->get('operators')->num_rows;
    ...
    function deletarRelacionamentooperator($id_operator) {

        $this->db->where('id_operator', $id_operator);
        $this->db->delete('department_rel_operator');
    }

    function salvarRelacionamentooperatorAndDepartamento($lista_id_department, $lista_main, $id_operator, $id_company) {
    ...
    function getoperatorByFacebookid($data) {
